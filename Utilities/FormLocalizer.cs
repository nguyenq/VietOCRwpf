...
namespace VietOCR.NET.Utilities
{
    class FormLocalizer
    {
        ...
        public FormLocalizer(Window form, Type formType)
        {
            this.form = form;
        ...
        public void ApplyCulture(CultureInfo culture)
        {
            ...
            foreach (FieldInfo fieldInfo in fieldInfos)
            {
                ...
                if (fieldType.IsSubclassOf(typeof(Control)) || fieldType.IsSubclassOf(typeof(MenuItem)))
                {
                    ...
                    if (fieldType.GetProperty("Header", typeof(object)) != null)
                    {
                        text = resources.GetString(fieldInfo.Name + ".Text");
                        if (text != null)
                        {
                            fieldType.InvokeMember("Header",
                                BindingFlags.SetProperty, null,
                                fieldInfo.GetValue(form), new object[] { text });
                        }
                    }
                    else if (fieldType.GetProperty("Content", typeof(object)) != null)
                    {
                        text = resources.GetString(fieldInfo.Name + ".Text");
                        if (text != null)
                        {
                            fieldType.InvokeMember("Content",
                                BindingFlags.SetProperty, null,
                                fieldInfo.GetValue(form), new object[] { text });
                        }
                    }
